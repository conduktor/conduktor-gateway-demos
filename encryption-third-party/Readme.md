...
[![asciicast](https://asciinema.org/a/3HQmE1bdZoHpvdVIj5PZbMpvt.svg)](https://asciinema.org/a/3HQmE1bdZoHpvdVIj5PZbMpvt)
...
security.protocol=SASL_PLAINTEXT
sasl.mechanism=PLAIN
sasl.jaas.config=org.apache.kafka.common.security.plain.PlainLoginModule required username='sa' password='eyJhbGciOiJIUzI1NiJ9.eyJ1c2VybmFtZSI6InNhIiwidmNsdXN0ZXIiOiJ0ZWFtQSIsImV4cCI6MTcxMzczNzk0Nn0.5EnfT-o6GvKyKrWBlu-OiHysSQ21BdoEo8gpxz6HNnc';
bootstrap.servers=localhost:6969
```
...
<details>
  ...
  ![Let's create a service account `third-party` for `teamA` virtual cluster](images/step-07-CREATE_VIRTUAL_CLUSTER.gif)
...
token=$(curl \
    ...
    --silent \
    --data-raw '{"lifeTimeSeconds": 7776000}' | jq -r ".token")
curl     --request POST "http://localhost:8888/admin/vclusters/v1/vcluster/teamA/username/third-party"     --header 'Content-Type: application/json'     --user 'admin:conduktor'     --silent     --data-raw '{"lifeTimeSeconds": 7776000}' | jq -r ".token"
...
<details>
  ...
  ![Creating topic `customers` on `teamA`](images/step-08-CREATE_TOPICS.gif)
...
<details>
  ...
  ![Adding interceptor `encrypt-on-consume`](images/step-09-ADD_INTERCEPTOR.gif)
...
Confirm `tom` and `florent` data is not encrypted for `teamA` in cluster `teamA`
...
Confirm `tom` and `florent` data is encrypted for `third-party` in cluster `teamA`
