...
[![asciicast](https://asciinema.org/a/rbXEziLUWezDi80fB60OG3oaI.svg)](https://asciinema.org/a/rbXEziLUWezDi80fB60OG3oaI)
...
docker compose up --detach --wait
 Network sql-topics-single-tenant_default  Creating
 Network sql-topics-single-tenant_default  Created
...
Creating virtual cluster `passthrough` on gateway `gateway1`
...
<details>
  ...
  ![Creating virtual cluster `passthrough`](images/step-05-CREATE_VIRTUAL_CLUSTER.gif)
...
token=$(curl \
    ...
    --silent \
    --data-raw '{"lifeTimeSeconds": 7776000}' | jq -r ".token")
curl     --request POST "http://localhost:8888/admin/vclusters/v1/vcluster/passthrough/username/sa"     --header 'Content-Type: application/json'     --user 'admin:conduktor'     --silent     --data-raw '{"lifeTimeSeconds": 7776000}' | jq -r ".token"
...
### Review the kafka properties to connect to `passthrough`
...
security.protocol=SASL_PLAINTEXT
sasl.mechanism=PLAIN
sasl.jaas.config=org.apache.kafka.common.security.plain.PlainLoginModule required username='sa' password='eyJhbGciOiJIUzI1NiJ9.eyJ1c2VybmFtZSI6InNhIiwidmNsdXN0ZXIiOiJwYXNzdGhyb3VnaCIsImV4cCI6MTcxMzc1MDUyMn0.yRNGbStpn2YBrN-AE54EqaMNEvrA8_b3mMD01O6z3e4';
bootstrap.servers=localhost:6969
```
...
<details>
  ...
  ![Creating topic `cars` on `passthrough`](images/step-07-CREATE_TOPICS.gif)
...
<details>
  ...
  ![Listing interceptors for `passthrough`](images/step-11-LIST_INTERCEPTORS.gif)
...
docker compose down --volumes
 ...
 Network sql-topics-single-tenant_default  Removing
 Network sql-topics-single-tenant_default  Removed
